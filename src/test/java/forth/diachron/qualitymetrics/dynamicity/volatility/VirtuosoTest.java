	...
	public static void getTotal() {
       ...
       public static void main(String[] args) {
		...
		RepositoryConnection con = null;
                String  query = "prefix co:<http://www.diachron-fp7.eu/changes/>" +
                "select ?a, ?x from <http://www.diachron-fp7.eu/changes> where {" +
                "?sc a co:Add_Label;\n" +
                "    co:al_p1 [co:al_v1 ?a];" +
                "    co:al_p2 [co:al_v2 ?x]." +
                "filter not exists {?cc co:consumes ?sc}" +
                "}";
                
                String query_t2 = "select ?instance ?simple_change ?nversion ?oversion "
			...
			con.setAutoCommit(true);
                      //  doTupleQuery (con, query_t2);
                        countTupleQueryResult(con, query_t2);
                }
                catch (Exception e) {
				log("Error[" + e + "]");
				e.printStackTrace();
	...
	private static Value[][] doTupleQuery(RepositoryConnection con, String query) throws RepositoryException, MalformedQueryException, QueryEvaluationException {
		...
		for (row = 0; bindings.hasNext(); row++) {
			...
			results.add(rv);
                ...
                System.out.println("VECTOR RESULTS length:" +results.size());
        ...
        private static int countTupleQueryResult(TupleQueryResult result)
         ...
         private static int countTupleQueryResult(RepositoryConnection con, String query)
             ...
             TupleQuery resultsTable = con.prepareTupleQuery(QueryLanguage.SPARQL, query);
             TupleQueryResult result = resultsTable.evaluate();
		  int count = 0;
		  while (result.hasNext()) {
		    result.next();
		    count++;
