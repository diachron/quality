...
public class Dereferencibility implements QualityMetric{
	...
	public void compute(Quad quad) {
		...
		if (HTTPConnector.isPossibleURL(subject) && (!CommonDataStructures.uriExists(subject.getURI()))){
			...
			if (profile.getHttpStatusCode() == 0) this.dereferencabilityChecker(this.buildURIProfile(subject, profile));
...
//		}
	...
	private URIProfile buildURIProfile(Node node, URIProfile p){
		...
		try {
			HTTPConnectorReport report = HTTPConnector.connectToURI(node, false); // We want to make sure that there is no content redirection, thus 3xx codes are reported
